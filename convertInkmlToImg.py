...
def get_label(inkml_file_abs_path):
    ...
    for child in root:
        if (child.tag == doc_namespace + 'annotation') and (child.attrib == {'type': 'UI'}):
            return child.text
...
if __name__ == '__main__':
    ...
    dim = 20
    padding = 4
    dir = '/media/lx/mydata/手写公式数据集/CROHME2019_data/Task1_onlineRec/subTask_symbols/Train/isolatedTrain2014/trainingSymbols'
    shutil.copy('/media/lx/mydata/手写公式数据集/CROHME2019_data/Task1_onlineRec/subTask_symbols/Train/isolatedTrain2014/trainingSymbols/iso_GT.txt','iso_GT.txt')
    save_dir='data_png_'
    save_dir_txt = 'data_txt_'
    files = os.listdir(dir)
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)
    if not os.path.isdir(save_dir_txt):
        os.makedirs(save_dir_txt)
    count = 0
    total = len(files)
    for file in files:
        if not file.endswith('.inkml'):
            print('\t\t\t\t'+file)
            continue

        img_path = save_dir+'/'+file.split('.')[0]+'.png'
        inkml_path = os.path.join(dir,file)
        if not os.path.isfile(inkml_path):
            print("\n\nInkml file not found:\n\t{}".format(inkml_path))
            continue

        traces = parse_inkml(inkml_path)
        ...
        imsave(img_path, im)
        if count%1000==0:
            print("{}/{}__{} is finished".format(count,total,file))
        count += 1
        fout = open(save_dir_txt+'/'+file.split('.')[0] + '.txt', 'w+')
        label = get_label(inkml_path)
        fout.write(label)
        fout.close()
    print("\n\n inkml to png is  Finished\n\n")


    ########### get pic_path---label
    labels=[]
    symbols=[]
    with open('iso_GT.txt','r') as file:
    ...
    for txt_path in txt_paths:
        with open(os.path.join(txt_dir,txt_path), 'r') as file:
            label = file.read()
        # if len(label)==0:
        #     print(txt_path)
        #     continue
        ...
        pic_name = txt_path.rstrip('.txt')+'.png'
        with open('symbol.txt','a') as file:
            file.write(pic_name+'\t'+symbol+'\n')
    shutil.rmtree("data_txt_")
    os.rename("data_png_","symbol_data")
